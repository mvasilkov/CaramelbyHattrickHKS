...
# define STB_TEXTEDIT_UNDOSTATECOUNT (XS_RANDOM_SIZE / 4)
# define STB_TEXTEDIT_UNDOCHARCOUNT (XS_RANDOM_SIZE / 2)
...
{
	if (!STB_TEXT_HAS_SELECTION(state)) state->select_start = state->select_end = state->cursor;
...
retry:
	...
	case STB_TEXTEDIT_K_LINESTART:
		...
		else while (state->cursor > 0 && STB_TEXTEDIT_GETCHAR(str, state->cursor - 1) != '\n') --state->cursor;
		state->has_preferred_x = 0;
		break;
	case STB_TEXTEDIT_K_LINEEND: {
		...
		else while (state->cursor < n && STB_TEXTEDIT_GETCHAR(str, state->cursor) != '\n') ++state->cursor;
		state->has_preferred_x = 0;
		break;
	}
...
void stb_textedit_discard_redo(StbUndoState *state)
{
	...
	if (state->redo_point <= k) {
		if (state->undo_rec[k].char_storage >= 0) {
			...
			memmove(state->undo_char + state->redo_char_point, state->undo_char + state->redo_char_point - n, (size_t)((size_t)(STB_TEXTEDIT_UNDOCHARCOUNT - state->redo_char_point) * sizeof(STB_TEXTEDIT_CHARTYPE)));
			for (i = state->redo_point; i < k; ++i)
				if (state->undo_rec[i].char_storage >= 0) state->undo_rec[i].char_storage = state->undo_rec[i].char_storage + (short)n;
		}
		memmove(state->undo_rec + state->redo_point, state->undo_rec + state->redo_point - 1, (size_t)((size_t)(STB_TEXTEDIT_UNDOSTATECOUNT - state->redo_point) * sizeof(state->undo_rec[0])));
		++state->redo_point;
	}
}
